...
public class FileDownloadService implements FileDownloadUsecase {
    ...
    @Override
    public DownloadResult download(BaseFile.TrialCondition condition) throws IOException {
        BaseFile baseFile = readFilePersistencePort.get(condition);

        return  fileDownloadPort.get(baseFile);
    }
}
